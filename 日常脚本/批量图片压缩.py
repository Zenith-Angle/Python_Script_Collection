...
def compress_image(input_image_path, output_image_path, max_size=None):
    ...
    try:
        ...
        img = Image.open(input_image_path)  # 必须重新打开图像
    except Exception as e:
        logging.error(f"无法打开图像 {input_image_path}：{e}")
        print(f"无法打开图像 {input_image_path}：{e}")
        return  # 跳过此文件
    ...
    if format not in ['JPEG', 'PNG']:
        try:
            ...
            logging.error(f"保存图像 {output_image_path} 时出错：{e}")
        return
...
def compress_task(image_file, input_dir, input_basename, output_dir,
                  max_size_user_defined, scaling_factor, progress_queue, pause_event,
                  stop_event):
    """
    压缩单张图像并更新进度队列。
    """
    ...
    if not os.path.exists(output_root):
        try:
            ...
            logging.error(f"创建目录 {output_root} 时出错：{e}")
            return
    ...
    if scaling_factor is not None:
        ...
        try:
            ...
            logging.error(f"无法获取文件大小 {image_file}：{e}")
            return
    else:
    ...
    progress_queue.put(1)  # 每完成一张图片，放入1
...
def compression_process(image_files, input_dir, input_basename, output_dir,
    ...
    for image_file in image_files:
        ...
        if stop_event.is_set():
            break
        compress_task(image_file, input_dir, input_basename, output_dir,
                      max_size_user_defined, scaling_factor, progress_queue, pause_event,
                      stop_event)
    # 压缩完成的标志
...
def main():
    ...
    root = tk.Tk()
    root.title("图片压缩工具")
    root.geometry("600x500")  # 调整窗口大小以适应所有控件
    root.resizable(False, False)

    # 应用 ttk 样式
    style = ttk.Style()
    try:
        style.theme_use('vista')  # 尝试使用 'clam' 主题，其他可选 'vista', 'default' 等
    except:
        pass  # 如果主题不可用，使用默认主题
    ...
    ttk.Label(content_frame, text="输入文件夹：").grid(row=0, column=0, padx=5, pady=5,
                                                      sticky='e')
    ttk.Entry(content_frame, textvariable=input_dir_var, width=40).grid(row=0, column=1,
                                                                        padx=5, pady=5)
    ttk.Button(content_frame, text="选择", command=select_input_dir).grid(row=0, column=2,
                                                                          padx=5, pady=5)

    # 输出文件夹选择
    output_dir_var = tk.StringVar()
    ttk.Label(content_frame, text="输出文件夹：").grid(row=1, column=0, padx=5, pady=5,
                                                      sticky='e')
    ttk.Entry(content_frame, textvariable=output_dir_var, width=40).grid(row=1, column=1,
                                                                         padx=5, pady=5)
    ttk.Button(content_frame, text="选择", command=select_output_dir).grid(row=1,
                                                                           column=2,
                                                                           padx=5, pady=5)

    # 压缩选项
    compression_option_var = tk.StringVar(value='size')
    ttk.Label(content_frame, text="压缩方式：").grid(row=2, column=0, padx=5, pady=5,
                                                    sticky='e')
    size_radiobutton = ttk.Radiobutton(content_frame, text='按最大文件大小压缩',
                                       variable=compression_option_var,
                                       value='size', command=update_compression_option)
    size_radiobutton.grid(row=2, column=1, padx=5, pady=5, sticky='w')
    scale_radiobutton = ttk.Radiobutton(content_frame, text='按比例压缩',
                                        variable=compression_option_var,
                                        value='scale', command=update_compression_option)
    scale_radiobutton.grid(row=3, column=1, padx=5, pady=5, sticky='w')

    # 最大文件大小输入
    max_size_var = tk.StringVar(value="1")  # 默认1MB
    ttk.Label(content_frame, text="最大文件大小（MB）：").grid(row=4, column=0, padx=5,
                                                            pady=5, sticky='e')
    max_size_entry = ttk.Entry(content_frame, textvariable=max_size_var, width=20)
    max_size_entry.grid(row=4, column=1, padx=5, pady=5, sticky='w')

    # 缩放比例下拉框
    scaling_factor_var = tk.StringVar(value="1/2")
    ttk.Label(content_frame, text="比例：").grid(row=5, column=0, padx=5, pady=5,
                                                sticky='e')
    scaling_options = ["1/2", "1/3", "1/4", "1/5", "1/10"]
    scaling_factor_combo = ttk.Combobox(content_frame, textvariable=scaling_factor_var,
                                        values=scaling_options,
                                        width=17, state='disabled')
    scaling_factor_combo.grid(row=5, column=1, padx=5, pady=5, sticky='w')

    # 进度条和状态标签
    progress_label_var = tk.StringVar(value="等待开始...")
    progress_label = ttk.Label(content_frame, textvariable=progress_label_var)
    progress_label.grid(row=6, column=0, columnspan=3, pady=10)

    progress_bar = ttk.Progressbar(content_frame, orient='horizontal', length=400,
                                   mode='determinate')
